...
def _get_iedges2(iedge: int, nedges: int) -> tuple[int, int, int]:
    values = np.arange(nedges)
    #i1 = values[(iedge) % nedges]
...
def reduce_line_nsm(points,
    ...
    #     pa, pb = edge
    #     i = pb - pa
    #     j[iedge, :] = np.cross(k, i)
    # j2

    nedges = len(points)
    points2 = points.copy()
    for iedge, p1 in enumerate(points):
        i2, i3, i4 = _get_iedges2(iedge, nedges)
        i2b, i3b, i4b = _get_iedges2(iedge, nedges)
        assert i2 == i2b
        assert i3 == i3b
        assert i4 == i4b
        p2 = points[i2]
